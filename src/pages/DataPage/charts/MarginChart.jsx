...
const MarginChart = () => {
  const [marginData, setMarginData] = useState([]); // Para almacenar el margen mensual
  ...
  useEffect(() => {
    const fetchMarginData = async () => {
      ...
      try {
        ...
        // Calcular el margen por mes
        const margenPorMes = ingresosPorMes.map((ingreso, index) => {
          const gasto = gastosPorMes[index];
          // Si los ingresos son cero, el margen será cero para evitar división por cero
          return ingreso > 0 ? ((ingreso - gasto) / ingreso) * 100 : 0;
        });

        setMarginData(margenPorMes);
      } catch (error) {
  ...
  const annotations = marginData.map((value, index) => ({
    x: months[index],
    y: value,
    text: `${value.toFixed(2)}%`, // Mostrar el margen en porcentaje
    xanchor: 'center',
    yanchor: 'bottom',
    showarrow: false,
    font: {
  ...
  // Definir los colores de las barras (verde para margen positivo, rojo para negativo)
  const barColors = marginData.map(value => value >= 0 ? '#28A745' : '#FF5733');

  return (
    <Box sx={{ padding: '20px', backgroundColor: '#ffffff', borderRadius: '16px', boxShadow: '0px 8px 16px rgba(0, 0, 0, 0.1)', mt: 6 }}>
      {/* Gráfico de margen mensual */}
      <Plot
        data={[
          {
            x: months,
